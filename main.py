...
class MultidimensionalTransdimensionalSplitter(QCAlgorithm):
  ...
  def OnData(self, data):
    '''OnData event is the primary entry point for your algorithm. Each new data point will be pumped
        Arguments:
         data: Slice object keyed by symbol cantaining the stock data
    '''
    self.Plot("Data Chart", self.symbol, self.Securities[self.symbol].Close)
  
  def EveryMarketOpen(self):
    close = slef.History(self.symbol, 31, Resolution.Daily)["close"]
